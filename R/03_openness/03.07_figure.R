...
fam_cor <- read_csv(sprintf("%s/%s/03.04_fam_cor_openness.csv", wd_noa, wd_noa_data)) %>%
...
est <- read.csv(sprintf("%s/SI/03.06_table_est_openness.csv", wd_oa)) %>%
  mutate(trait = "Openness")
est_0 <- read.csv(sprintf("%s/SI/03.06_table_est_0tham_openness.csv", wd_oa)) %>%
  mutate(trait = "Height")
...
panel_a <- make_panel_a(
  ...
  fill_values   = c("dAM-ADE" = "#E59F00", "iAM-ADE" = "lightgray", "Observed" = "#0c72b2"),
...
plot_oe <- (panel_a + panel_b) +
  plot_layout(widths = c(2, 1)) +
  plot_annotation(tag_levels = "A")

# SAVE ####
ggsave(
  filename = sprintf("%s/%s/03.07.FIG_3.pdf", wd_oa, wd_oa_figures),
  plot     = plot_oe,
  width    = 12.5,
  height   = 3.25,
  device   = cairo_pdf
