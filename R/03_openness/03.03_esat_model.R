...
q1 <- quantile(dat_fam2g_long %>% pull(value), .25, na.rm = T)
q3 <- quantile(dat_fam2g_long %>% pull(value), .75, na.rm = T)
iqr <- IQR(dat_fam2g_long %>% pull(value), na.rm = T)

nrow(dat_fam2g_long)
# 44725 
dat_fam2g_long %>% filter(
  !(value > (q1 - 3*iqr) & value < (q3 + 3*iqr))
) %>% select(twzyg, sex, value)
# there is no extreme outlier

# TRANSFORMATION & RESIDUALIZATION ####s
...
write_csv(n_compair, sprintf("%s/%s/03.03_pair_number_openness.csv", wd_noa, wd_noa_data))
...
save(esat_fit, file = sprintf("%s/%s/03.03_saturated_openness.Rdata", wd_noa, wd_noa_data))
...
esat_msib1_fit <- mxTryHard(esat_msib1_fit, intervals = F)
mxCompare(esat_fit, esat_msib1_fit) # we cannot equate twin and sibling means 
esat_msib1_sumy <- summary(esat_msib1_fit)
esat_msib1_sumy$parameters

# equate variances across zygosities
esat_vartw1_mod <- mxModel(esat_mtw1_mod, name = "var across zyg and sib")
...
esat_mean1_fit <- mxTryHard(esat_mean1_mod, intervals = F)
mxCompare(esat_fit, esat_mean1_fit)

# equate variances
esat_var1_mod <- mxModel(esat_mean1_mod, name = "var partners and sib")
...
# extract parameters
esat_var1_sumy <- summary(esat_var1_mod)
...
esat_law_fit <- mxRun(esat_law_mod, intervals = T)
...
# SAVE --------------------------------------------------------------
save(esat_am2_fit, file = sprintf("%s/%s/03.03_saturated_reduced_openness.Rdata", wd_noa, wd_noa_data))
...
## supplementary table ####
write_csv(as.data.frame(esat_model_comparsion), sprintf("%s/SI/03.03_table_esat_mxcompare_openness.csv", wd_oa))