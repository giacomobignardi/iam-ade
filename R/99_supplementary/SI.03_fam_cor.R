...
load(file = sprintf("%s/%s/SI.02_saturated_reduced_EA.Rdata", wd_noa, wd_noa_data))

# load number of complete pairs
n_compair <- read.csv(sprintf("%s/%s/SI.02_pair_number_EA.csv", wd_noa, wd_noa_data))
...
# SAVE ####
write_csv(fam_cor_fin, sprintf("%s/%s/SI.03_fam_cor_EA.csv", wd_noa, wd_noa_data))
write_csv(fam_est, sprintf("%s/%s/SI.03_table_covr_EA.csv", wd_noa, wd_noa_data))
