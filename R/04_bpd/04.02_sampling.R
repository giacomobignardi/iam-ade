...
# -5   -4   -3   -2   -1    0    1    2    3    4 
# 2    2   61   72  302 8219  272   62   56    4  
nrow(dat_twf[which((dat_twf$age_tw1 - dat_twf$age_tw2) != 0),])
# for 833 twins age was different, with a maximum difference of 5 years apart
...
# SAVE --------------------------------------------------------------
# demographic
write_csv(dem, sprintf("%s/SI/04.02_table_dem_bpd.csv", wd_oa))

## sampled family twin data ####
write_csv(dat_twfam, sprintf("%s/%s/04.02_dat_estd_bpd.csv", wd_noa, wd_noa_data))
