...
dat <- dat %>% rename(lengt_7 = lengt7, lengt_8 = lengt8, lengt_10 = lengt10)
var_name <- "lengt"
...
# -5   -4   -3   -2   -1    0    1    2    3    4 
# 1    5   53   61  278 8391  273   55   50    4 
nrow(dat_twf[which((dat_twf$age_tw1 - dat_twf$age_tw2) != 0),])
# for 780 twins age was different, with a maximum difference of 5 years apart
...
# SAVE ####
# demographic
write_csv(dem, sprintf("%s/SI/02.02_table_dem_height.csv", wd_oa))

## sampled family twin data ####
write_csv(dat_twfam, sprintf("%s/%s/02.02_dat_estd_height.csv", wd_noa, wd_noa_data))
