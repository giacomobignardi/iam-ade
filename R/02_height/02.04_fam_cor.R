...
load(file = sprintf("%s/%s/02.03_saturated_reduced_height.Rdata", wd_noa, wd_noa_data))

# load number of complete pairs
n_compair <- read.csv(sprintf("%s/%s/02.03_pair_number_height.csv", wd_noa, wd_noa_data))
...
# SAVE ####
write_csv(fam_cor_fin, sprintf("%s/%s/02.04_fam_cor_height.csv", wd_noa, wd_noa_data))
write_csv(fam_est, sprintf("%s/SI/02.04_table_covr_height.csv", wd_oa))
