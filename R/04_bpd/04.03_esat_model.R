...
dat_fam2g_long %>% group_by(twzyg,rel) %>% reframe(mean(sqrtres, na.rm =T))
...
write_csv(n_compair, sprintf("%s/%s/04.03_pair_number_bpd.csv", wd_noa, wd_noa_data))
...
save(esat_fit, file = sprintf("%s/%s/04.03_saturated_bpd.Rdata", wd_noa, wd_noa_data))
...
mxCompare(esat_fit, esat_mbo_fit) # we can constrain means

# constrain means and variances across  pairs
esat_mvar_mod <- mxModel(esat_mbo_fit, name = "means and variances order")
...
esat_mtw1_mod <- omxSetParameters(esat_mtw1_mod, label = c("mmz", "mdz"), free = TRUE, values = 1, newlabels = "msib")
esat_mtw1_fit <- mxRun(esat_mtw1_mod, intervals = F)
mxCompare(esat_fit, esat_mtw1_fit) # we cannot equate mean across zygosities
esat_mtw1_sumy <- summary(esat_mtw1_fit)
esat_mtw1_sumy$parameters

# equate variances across zygosities
esat_vartw1_mod <- mxModel(esat_mvar_mod, name = "var across zyg ")
esat_vartw1_mod <- omxSetParameters(esat_vartw1_mod, label = c("vmz", "vdz"), free = TRUE, values = 1, newlabels = "vtw")
esat_vartw1_fit <- mxRun(esat_vartw1_mod, intervals = F) 
mxCompare(esat_fit, esat_vartw1_fit)

# equate variances across zygosities
esat_varsib1_mod <- mxModel(esat_vartw1_mod, name = "var zyg and sib")
esat_varsib1_mod <- omxSetParameters(esat_varsib1_mod, label = c("vtw", "vfs"), free = TRUE, values = 1, newlabels = "vsib")
esat_varsib1_fit <- mxRun(esat_varsib1_mod, intervals = F) 
mxCompare(esat_fit, esat_varsib1_fit) # we cannot equate mean across zygosities and sibs

# equate mean and variances across zygosities for spouses of twins
esat_mvastw_mod <- mxModel(esat_varsib1_mod, name = "mean and var across zyg for partners")
...
# extract parameters
esat_var1_sumy <- summary(esat_var1_fit)
esat_var1_sumy$parameters

# equate means
esat_mean1_mod <- mxModel(esat_var1_mod, name = "mean partners and sib")
esat_mean1_mod <- omxSetParameters(esat_mean1_mod, label = c("mstw", "mfs","mmz"), free = TRUE, values = 1, newlabels = "mp")
...
# SAVE --------------------------------------------------------------
save(esat_am2_fit, file = sprintf("%s/%s/04.03_saturated_reduced_bpd.Rdata", wd_noa, wd_noa_data))
...
## supplementary table ####
write_csv(as.data.frame(esat_model_comparsion), sprintf("%s/SI/04.03_table_esat_mxcompare_bpd.csv", wd_oa))