...
write_csv(n_compair, sprintf("%s/%s/02.03_pair_number_height.csv", wd_noa, wd_noa_data))
...
save(esat_fit, file = sprintf("%s/%s/02.03_saturated_height.Rdata", wd_noa, wd_noa_data))
...
mxCompare(esat_fit, esat_mbo_fit) # we cannot constrain means -> BIRTH ORDER EFFECT

# constrain means and variances across  pairs
esat_mvar_mod <- mxModel(esat_fit, name = "means and variances order")
...
esat_mtw1_mod <- omxSetParameters(esat_mtw1_mod, label = c("mmz1", "mdz1"), free = TRUE, values = 1, newlabels = "msib1")
esat_mtw1_mod <- omxSetParameters(esat_mtw1_mod, label = c("mmz2", "mdz2"), free = TRUE, values = 1, newlabels = "msib2")
esat_mtw1_fit <- mxRun(esat_mtw1_mod, intervals = F)
esat_mtw1_fit <- mxTryHard(esat_mtw1_fit, intervals = F)
mxCompare(esat_fit, esat_mtw1_fit) # we cannot equate mean across zygosities -> ZYGOSITY EFFECT
...
# esat_am2_fit <- mxTryHard(esat_am2_mod, intervals = F)
mxCompare(esat_law_fit, esat_am2_fit)

# SAVE ####
save(esat_am2_fit, file = sprintf("%s/%s/02.03_saturated_reduced_height.Rdata", wd_noa, wd_noa_data))
...
## supplementary table ####
write_csv(as.data.frame(esat_model_comparsion), sprintf("%s/SI/02.03_table_esat_mxcompare_height.csv", wd_oa))
