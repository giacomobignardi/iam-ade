...
q1_male <- quantile(dat_fam2g_long %>% filter(sex == 1) %>% pull(value), .25, na.rm = T)
q3_male <- quantile(dat_fam2g_long %>% filter(sex == 1) %>% pull(value), .75, na.rm = T)
iqr_male <- IQR(dat_fam2g_long %>% filter(sex == 1) %>% pull(value), na.rm = T)

q1_female <- quantile(dat_fam2g_long %>% filter(sex == 2) %>% pull(value), .25, na.rm = T)
q3_female <- quantile(dat_fam2g_long %>% filter(sex == 2) %>% pull(value), .75, na.rm = T)
iqr_female <- IQR(dat_fam2g_long %>% filter(sex == 2) %>% pull(value), na.rm = T)

nrow(dat_fam2g_long)
# 45835 
dat_fam2g_long %>% filter(
  !((sex == 1 & value > (q1_male - 3*iqr_male) & value < (q3_male + 3*iqr_male)) | (sex == 2 & value > (q1_female - 3*iqr_female) & value < (q3_female + 3*iqr_female)))
) %>% select(twzyg, sex, value)
# twzyg     sex value
# 1 DZ        2   131
# 2 DZ        2   205
# 3 DZ        2   138
...
# we removed these 5 individuals
dat_fam2g_long <- dat_fam2g_long %>% filter(
  (sex == 1 & value > (q1_male - 3*iqr_male) & value < (q3_male + 3*iqr_male)) | 
    (sex == 2 & value > (q1_female - 3*iqr_female) & value < (q3_female + 3*iqr_female)) |
    is.na(value)
)
nrow(dat_fam2g_long)
...
load(sprintf("%s/%s/02.03_saturated_height.Rdata", wd_noa, wd_noa_data))
load(sprintf("%s/%s/02.03_saturated_reduced_height.Rdata", wd_noa, wd_noa_data))
...
mean_MZ <- c("mean_STW", "mean_MZ1", "mean", "mean_MZ2", "mean_STW")
mean_DZ <- c("mean_STW", "mean_DZ1", "mean", "mean_DZ2", "mean_STW")

# ACE ####
# first fit classical twin design
# with dominance
ace_mod <- acde.fun(mz, dz, c("value_tw1", "value_tw2"), mean.MZ = c("meanMZ1", "meanMZ2"),  mean.DZ = c("meanDZ1", "meanDZ2"), cp.free = T)
ace_fit <- mxRun(ace_mod, intervals = T)
ace_fit$US$result

# without shared environmental effects
ae_mod <- acde.fun(mz, dz, c("value_tw1", "value_tw2"), mean.MZ = c("meanMZ1", "meanMZ2"),  mean.DZ = c("meanDZ1", "meanDZ2"))
...
# Save table CTD
write_csv(ae_est %>% select(model, parameter, estimate, lbound, ubound) %>% mutate(lbound = round(lbound, 5), estimate = round(estimate, 5), ubound = round(ubound, 5)), sprintf("%s/SI/02.06_tableCTD_est_height.csv", wd_oa))
...
# mxGetExpected(dxam_ade_fit, "covariance")$MZ-cov(mz, use = "pairwise.complete.obs")
# mxGetExpected(dxam_ade_fit, "covariance")$DZ-cov(dz, use = "pairwise.complete.obs")
# mxGetExpected(dxam_ade_fit, "covariance")$MZ-mxGetExpected(esat_law_fit, "covariance")$mz
# mxGetExpected(dxam_ade_fit, "covariance")$DZ-mxGetExpected(esat_law_fit, "covariance")$dz

# model of interest (re-run to obtain confidence intervals)
iam_results_diagnostic %>% filter(model_name == "xdam_ade")
iam_results_diagnostic %>% filter(model_name == "dam_ade")

xdam_ade_fit <- mxTryHard(xdam_ade_mod, intervals = T)
dam_ade_fit  <- mxRun(dam_ade_mod, intervals = T)

xdam_ade_est <- summary(xdam_ade_fit)$CI %>%
  as.data.frame() %>%
  mutate(parameter = c(colnames(xdam_ade_fit$est_path$result), colnames(xdam_ade_fit$est_var$result), colnames(xdam_ade_fit$est_cov$result)), model = "xdAM-ADE")
...
est <- rbind(xdam_ade_est, dam_ade_est)

# Save table 4
write_csv(est %>% filter(!parameter %in% c("varP_N", "varS_N", "n1s", "n1", "rN_Partner", "rN_FS")) %>% select(model, parameter, estimate, lbound, ubound) %>% mutate(lbound = round(lbound, 5), estimate = round(estimate, 5), ubound = round(ubound, 5)), sprintf("%s/SI/02.06_table_est_height.csv", wd_oa))
...
write_csv(xam_cdate_mxcompare, sprintf("%s/SI/02.06_table_mod_comp_height.csv", wd_oa))
...
xdam_ader_mod <- iam.date.fun(mz, dz, sel_vars,
...
xam_ace_mod <- iam.date.fun(mz, dz, sel_vars,
                           indirect.assortment = F,
                           mean.MZ = mean_MZ, mean.DZ = mean_DZ,
...
xam_ace_fit <- mxTryHard(xam_ace_mod, intervals = T)
xam_ace_fit$est_var
xam_ace_est <- summary(xam_ace_fit)$CI %>%
  as.data.frame() %>%
  mutate(parameter = c(colnames(xam_ace_fit$est_path$result), colnames(xam_ace_fit$est_var$result), colnames(xam_ace_fit$est_cov$result)), model = "ACE (0th gen am)")

# Save table 6
write_csv(xam_ace_est %>% filter(!parameter %in% c("varP_N", "varS_N", "n1s", "n1", "rN_Partner", "rN_FS")) %>% select(model, parameter, estimate, lbound, ubound) %>% mutate(lbound = round(lbound, 5), estimate = round(estimate, 5), ubound = round(ubound, 5)), sprintf("%s/SI/02.06_table_est_0tham_height.csv", wd_oa))

# rev1.R2.8 dominance under random mating ####
iam_results_diagnostic %>% filter(model_name == "ace")
ace_fit  <- mxRun(ace_mod, intervals = T)
ace_est <- summary(ace_fit)$CI %>%
  as.data.frame() %>%
  mutate(parameter = c(colnames(ace_fit$est_path$result), colnames(ace_fit$est_var$result), colnames(ace_fit$est_cov$result)), model = "ACE")

# Save table 7
write_csv(ace_est %>% filter(!parameter %in% c("varP_N", "varS_N", "n1s", "n1", "rN_Partner", "rN_FS")) %>% select(model, parameter, estimate, lbound, ubound) %>% mutate(lbound = round(lbound, 5), estimate = round(estimate, 5), ubound = round(ubound, 5)), sprintf("%s/SI/02.06_table_est_noam_height.csv", wd_oa))
...
for (i in seq(2, 10, 1)) {
  ...
  xdam_ane_fit <- mxTryHard(xdam_ane_mod, intervals = T)
  xdam_ane_est_i <- summary(xdam_ane_fit)$CI %>%
    as.data.frame() %>%
    mutate(parameter = c(colnames(xdam_ane_fit$est_path$result), colnames(xdam_ane_fit$est_var$result), colnames(xdam_ane_fit$est_cov$result)), model = "xdAM-AIE", ri = i)
  xdam_ane_est <- rbind(xdam_ane_est, xdam_ane_est_i)
}

# SAVE ####
# save for later
nate_est_tb <- xdam_ane_est %>%
...
# Save table 8
write_csv(nate_est_tb, sprintf("%s/SI/02.06_table_est_nonadditive_height.csv", wd_oa))