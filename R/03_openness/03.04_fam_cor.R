...
load(file = sprintf("%s/%s/03.03_saturated_reduced_openness.Rdata", wd_noa, wd_noa_data))

# load number of complete pairs
n_compair <- read.csv(sprintf("%s/%s/03.03_pair_number_openness.csv", wd_noa, wd_noa_data))
...
# SAVE ####
write_csv(fam_cor_fin, sprintf("%s/%s/03.04_fam_cor_openness.csv", wd_noa, wd_noa_data))
write_csv(fam_est, sprintf("%s/SI/03.04_table_covr_openness.csv", wd_oa))