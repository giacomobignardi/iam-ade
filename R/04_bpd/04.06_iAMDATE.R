...
dat_fam2g <- read.csv(sprintf("%s/%s/04.02_dat_estd_bpd.csv", wd_noa, wd_noa_data))
...
# TRANSFORMATION & EXCLUSION & RESIDUALIZATION ####
# here first we transform to align with previous study on BPD features in the same sample (Distel et al., 2009, "Because the data showed a somewhat skewed distribution with a tail to the right, a square root transformation was applied. ")
dat_fam2g_long <- dat_fam2g_long %>% mutate(sqrt = sqrt(value))

q3 <- quantile(dat_fam2g_long %>% pull(sqrt), .75, na.rm = T)
iqr <- IQR(dat_fam2g_long %>% pull(sqrt), na.rm = T)
dat_fam2g_long %>% filter(
  !(sqrt < (q3 + 3*iqr))
) %>% select(fid, twzyg, rel, sex, sqrt)

# we removed no inidivuals
dat_fam2g_long <- dat_fam2g_long %>% filter(sqrt < (q3 + 3*iqr) | is.na(sqrt))
nrow(dat_fam2g_long)
# 44860 

# residualise for sex and age
dat_fam2g_long$sqrtres <- scale(residuals(lm(sqrt ~ sex + age, data = dat_fam2g_long, na.action = na.exclude)))
ifelse(is_cluster == F, hist(dat_fam2g_long$sqrtres), print("no GUI in current env"))

# pivot wider
dat_fam2g <- dat_fam2g_long %>%
  dplyr::select(fid, twzyg, rel, sqrtres) %>%
  pivot_wider(names_from = rel, values_from = sqrtres)
dat_fam2g <- dat_fam2g %>% rename(value_tw1 = tw1, value_tw2 = tw2, value_s1 = s1, value_stw1 = stw1, value_stw2 = stw2)

# load saturated model for later model comparison
load(sprintf("%s/%s/04.03_saturated_bpd.Rdata", wd_noa, wd_noa_data))
load(sprintf("%s/%s/04.03_saturated_reduced_bpd.Rdata", wd_noa, wd_noa_data))
...
# with dominance
ade_mod <- acde.fun(mz, dz, c("value_tw1", "value_tw2"), mean.MZ = c("mean", "mean"),  mean.DZ = c("meanDZ", "meanDZ"), dp.free = T)
ade_fit <- mxRun(ade_mod, intervals = T)
ade_fit$US$result

# without shared environmental effects
ae_mod <- acde.fun(mz, dz, c("value_tw1", "value_tw2"), mean.MZ = c("mean", "mean"),  mean.DZ = c("meanDZ", "meanDZ"))
...
# Save table CTD
write_csv(ae_est %>% select(model, parameter, estimate, lbound, ubound) %>% mutate(lbound = round(lbound, 5), estimate = round(estimate, 5), ubound = round(ubound, 5)), sprintf("%s/SI/04.06_tableCTD_est_bpd.csv", wd_oa))
...
# Save table 4
write_csv(est %>% filter(!parameter %in% c("varP_N", "varS_N", "n1s", "n1", "rN_Partner", "rN_FS")) %>% select(model, parameter, estimate, lbound, ubound) %>% mutate(lbound = round(lbound, 5), estimate = round(estimate, 5), ubound = round(ubound, 5)), sprintf("%s/SI/04.06_table_est_bpd.csv", wd_oa))
...
write_csv(xam_cdate_mxcompare, sprintf("%s/SI/04.06_table_mod_comp_bpd.csv", wd_oa))
...
xam_ade_mod <- iam.date.fun(mz, dz, sel_vars,
                            indirect.assortment = F,
                            mean.MZ = mean_MZ, mean.DZ = mean_DZ,
                            d1.free = T, c1.free = F, t1.free = F,
                            d1s.free = T, c1s.free = F, t1s.free = F,
...
xam_ade_fit <- mxRun(xam_ade_mod, intervals = T)
xam_ade_fit$est_var
xam_ade_est <- summary(xam_ade_fit)$CI %>%
  as.data.frame() %>%
  mutate(parameter = c(colnames(xam_ade_fit$est_path$result), colnames(xam_ade_fit$est_var$result), colnames(xam_ade_fit$est_cov$result)), model = "ADE (0th gen am)")

# Save table 6
write_csv(xam_ade_est %>% filter(!parameter %in% c("varP_N", "varS_N", "n1s", "n1", "rN_Partner", "rN_FS")) %>% select(model, parameter, estimate, lbound, ubound) %>% mutate(lbound = round(lbound, 5), estimate = round(estimate, 5), ubound = round(ubound, 5)), sprintf("%s/SI/04.06_table_est_0tham_bpd.csv", wd_oa))

# ## rev1.R2.8 dominance under random mating ####
iam_results_diagnostic %>% filter(model_name == "ade")
ade_fit  <- mxRun(ade_mod, intervals = T)
ade_fit  <- mxTryHard(ade_fit, intervals = T)
...
# Save table 7
write_csv(ade_est %>% filter(!parameter %in% c("varP_N", "varS_N", "n1s", "n1", "rN_Partner", "rN_FS")) %>% select(model, parameter, estimate, lbound, ubound) %>% mutate(lbound = round(lbound, 5), estimate = round(estimate, 5), ubound = round(ubound, 5)), sprintf("%s/SI/04.06_table_est_noam_bpd.csv", wd_oa))
...
# Save table 8
write_csv(nate_est_tb, sprintf("%s/SI/04.06_table_est_nonadditive_bpd.csv", wd_oa))