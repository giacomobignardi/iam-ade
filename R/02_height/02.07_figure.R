...
mod_best <- "xdAM-ADE"
mod_comp <- "dAM-ADE"
mod_dieq <- "ACE (0th gen am)"

fam_cor <- read_csv(sprintf("%s/%s/02.04_fam_cor_height.csv", wd_noa, wd_noa_data)) %>%
  ...
  add_row(type = "ram_s", lbound = NA, estimate = NA, ubound = NA,
          trait = "Height", correlation = "Observed",
          title = "Resemblance between relatives")

est <- read.csv(sprintf("%s/SI/02.06_table_est_height.csv", wd_oa)) %>%
  mutate(trait = "Height")
est_0 <- read.csv(sprintf("%s/SI/02.06_table_est_0tham_height.csv", wd_oa)) %>%
  mutate(trait = "Height")
...
var <- rbind(est,est_0) %>%
  filter((parameter %in% c("varP_A","varP_D","varP_E","varS_A","varS_D","varS_E") & model %in% c(mod_best,mod_best)) |
         (parameter %in% c("varP_A","varP_C","varP_E","varS_A","varS_C","varS_E") & model == mod_dieq)) %>%
  mutate(
    trait = case_when(
...
panel_b <- make_panel_b(
  ...
  fill_colors  = c("#b41f01","#faa53c","#008B8B","#3e59dc")
)

plot_height <- (panel_a + panel_b) +
  plot_layout(widths = c(2, 1)) +
  plot_annotation(tag_levels = "A")

# SAVE ####
ggsave(
  filename = sprintf("%s/%s/02.07.FIG_3.pdf", wd_oa, wd_oa_figures),
  plot     = plot_height,
  width    = 12.5,
  height   = 3.25,
  device   = cairo_pdf
